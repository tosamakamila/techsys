...
用法：
    python scripts/scaffold_knowledge_map.py courses/<课程名>
    python scripts/scaffold_knowledge_map.py courses/<课程名> --dry-run  # 只预览不写文件
    python scripts/scaffold_knowledge_map.py courses/<课程名> --chapter 2  # 只生成指定章节
"""
...
def scaffold(course_dir: Path, target_chapter: int = 0, dry_run: bool = False):
    ...
    # 2. 扫描 transformed 目录
    transformed_dir = course_dir / "transformed"
    if not transformed_dir.exists():
        print(f"警告：{transformed_dir} 目录不存在，尝试从 materials 提取")
        concepts_by_fragment = extract_from_materials(course_dir, fragments)
    else:
        concepts_by_fragment = defaultdict(list)
        tf_files = sorted(transformed_dir.glob("*.md"))
        print(f"找到 {len(tf_files)} 个教案文件")

        for tf_path in tf_files:
            # 从文件名推断片段 ID（如 L001a_绪论上.md → L001a）
            stem = tf_path.stem
            frag_match = re.match(r"(L\d+[a-z]?)", stem)
            if not frag_match:
                continue
            fragment_id = frag_match.group(1)
            raw_concepts = extract_concepts_from_transformed(tf_path)
            for rc in raw_concepts:
                if rc and len(rc) >= 2:
                    concepts_by_fragment[fragment_id].append(rc)
            if raw_concepts:
                print(f"  {fragment_id}: 提取到 {len(raw_concepts)} 个概念")
...
def main():
    if len(sys.argv) < 2:
        ...
        print("      python scripts/scaffold_knowledge_map.py courses/<课程名> --dry-run")
        print("      python scripts/scaffold_knowledge_map.py courses/<课程名> --chapter 2")
        sys.exit(1)
